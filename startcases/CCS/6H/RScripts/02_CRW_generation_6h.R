...
out_dir <- paste0("C:/github/Whale-SDM/startcases/CCS/6H/CRW")
...
if (length(missing_columns) > 0) {
    stop(paste0("File ", basename(file), " is missing these columns: ", paste(missing_columns, collapse = ", ")))
  }
...
tags <- bind_rows(lapply(track_files, read_whale_track)
) %>%
filter(
  !is.na(tags),
  !is.na(long),
...
track_summary <- tags %>% group_by(tags) %>%
  ...
  end_time = max(dTime),
    ...
    median_interval_hours = if (n() > 1) {
      median(as.numeric(diff(dTime), units = "hours"),
        na.rm = TRUE
  )
    } else {
      NA_real_
...
land_map <- maps::map("worldHires", fill = TRUE, plot = FALSE,
  xlim = xlim_land,
  ylim = ylim_land
)

land_sf <- sf::st_as_sf(land_map)
  
#maps package coordinates are already in longitude/latitude.
...
#temporarily disable spherical geometry for these intersection operations
  old_s2 <- sf::sf_use_s2()
...
mean_lat <- mean(tag$lat, na.rm = TRUE)
...
tag_utm <- sf::st_transform(tag_sf, crs = utm_epsg)

tag_xy <- sf::st_coordinates(tag_utm)
...
sim_ll <- sf::st_transform(sim_sf,crs = 4326)
  
simulated_coordinates <- sf::st_coordinates(sim_ll)
...
sim_all <- bind_rows(simulation_list)
#write csv
whale_csv <- file.path(out_dir, paste0("crw_sim_", tagid, ".csv"))
write.csv(sim_all, whale_csv, row.names = FALSE)

#diagnostic plot
whale_png <- file.path(out_dir, paste0("crw_sim_tracks_", tagid, ".png"))
plot_iterations <- sort(unique(sim_all$iteration))
set.seed(9000 + sum(utf8ToInt(as.character(tagid))))
plot_iterations <- sample(plot_iterations, size = min(plot_n, length(plot_iterations)),
  replace = FALSE
)
...
plot_xlim <- range(
  c(
    ...
    sim_all$long),
  na.rm = TRUE
) + c(-plot_buffer, plot_buffer)

plot_ylim <- range(
  c(tag$lat, sim_all$lat),
  na.rm = TRUE
) + c(
  -plot_buffer,
  plot_buffer
...
all_crw <- bind_rows(all_crw_list)

combined_crw_file <- file.path(out_dir, "crw_sim_all_four_whales.csv")
...
#11. observed + available locations
...
model_locations_file <- file.path(out_dir, "observed_and_crw_all_four_whales.csv")

write.csv(model_locations, model_locations_file, row.names = FALSE)

#Quality summaries

cat("\n============================================\n")

cat("CRW generation complete\n")
...
print(observed_locations %>%
    count(whale_id, name = "observed_locations"))

cat("\nSimulated CRW rows by whale:\n")

print(
  all_crw %>%
    count(whale_id, name = "simulated_rows"))

cat("\nNumber of complete CRW tracks by whale:\n")

print(
  all_crw %>%
    distinct(whale_id, sim_id
    ) %>%
    count(
      whale_id,
