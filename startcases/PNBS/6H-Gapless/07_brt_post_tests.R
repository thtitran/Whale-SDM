...
# Paths
# ------------------------------------------------------------
setwd("C:/github/Whale-SDM/startcases/PNBS/6H-Gapless/BRT-output")

model_data_dir <- paste0("C:/github/Whale-SDM/", "startcases/PNBS/6H-Gapless/Model-data")

bundle_file <- file.path(model_data_dir, "BRT-ready", "brt_model_data_bundle.rds")

brt_output_dir <- paste0("C:/github/Whale-SDM/", "startcases/PNBS/6H-Gapless/BRT-output")
...
brt_output_dir <- "C:/github/Whale-SDM/startcases/PNBS/6H-gapless/BRT-output"
...
#boundary limits
plot_xlim <- range(
  all_choice_retention_spatial$long,
  na.rm = TRUE)

plot_ylim <- range(
  all_choice_retention_spatial$lat,
  na.rm = TRUE)

#Small visual buffer around the whale tracks
lon_buffer <- 0.15
lat_buffer <- 0.15

plot_xlim <- plot_xlim + c(
  -lon_buffer,
  lon_buffer)

plot_ylim <- plot_ylim + c(
  -lat_buffer,
  lat_buffer)

#create CA outline
california_map <- ggplot2::map_data("state") %>%
...
#Choices lost because dynamic data were incomplete
ggplot2::geom_point(
  data = dplyr::filter(
    all_choice_retention_spatial,
    !combined_retained
...
#Choices retained in combined model
ggplot2::geom_point(
  data = dplyr::filter(
    all_choice_retention_spatial,
    combined_retained
...
#plotting limits frm whale locations
ggplot2::coord_equal(
  xlim = plot_xlim,
  ylim = plot_ylim,
  expand = FALSE
...
ggplot2::ggsave(file.path(brt_output_dir, "Spatial_distribution_of_retained_choices.png"))

#Save result
readr::write_csv(spatial_extent_summary, file.path(brt_output_dir, "spatial_extent_summary.csv"))


print(spatial_extent_summary, width = Inf)
