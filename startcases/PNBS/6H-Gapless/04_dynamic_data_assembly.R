...
out_dir <- paste0("C:/github/Whale-SDM/", "startcases/PNBS/6H-Gapless/Model-data")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

#===================================
#1. load static-complete table
#===================================
static_file <- file.path("C:/github/Whale-SDM/startcases/PNBS/6H-Gapless/Model-data", "observed_crw_four_whales_static_complete.csv")
...
viirs_chla_file <- paste0("C:/github/Whale-SDM/data/VIIRSNOAA20GAPLESS/", "NOAA20_VIIRS_GL_chla_20240331_20240731_combined.nc")
viirs_kd490_file <- paste0("C:/github/Whale-SDM/data/VIIRS-kd490GAPLESS/", "VIIRS_Kd490_GL_20240331_20240731_combined.nc")
dynamic_files <- c(
  L3S_LEO = l3s_file,
  VIIRS_chla_GL = viirs_chla_file,
  VIIRS_Kd490_GL = viirs_kd490_file
)
...
chla_GL_r <- terra::rast(viirs_chla_file)
kd490_GL_r <- terra::rast(viirs_kd490_file)

#===================================
#4. Validate raster time metadata
#===================================
validate_daily_raster <- function(r, product_name)
{
  raster_time <- terra::time(r)
  if (length(raster_time) != terra::nlyr(r) || all(is.na(raster_time)))
  {
    stop(product_name, " has invalid or incomplete time metadata.")
  }
  raster_dates <- as.Date(raster_time)
  if (anyDuplicated(raster_dates)) {
    stop(product_name, " contains duplicated dates.")
  }
  return(raster_dates)
}
sst_dates <- validate_daily_raster(sst_r, "L3S SST")
sst_gradient_dates <- validate_daily_raster(sst_gradient_r, "L3S SST gradient")
sst_front_dates <- validate_daily_raster(sst_front_r, "L3S SST front")
chla_GL_dates <- validate_daily_raster(chla_GL_r, "NOAA-20 VIIRS chlorophyll-a")
kd490_GL_dates <- validate_daily_raster(kd490_GL_r, "VIIRS Kd490")
#The three L3S variables should have identical dates
if (!identical(as.character(sst_dates), as.character(sst_gradient_dates)) || !identical( as.character(sst_dates), as.character(sst_front_dates)))
{
  stop("The three L3S variables do not share the same date axis.")
}

#Check if products have proper dates
dynamic_model_pts <- dynamic_model_pts %>%
  mutate(
    l3s_date_available = date %in% sst_dates,
    chla_GL_date_available = date %in% chla_GL_dates,
    kd490_GL_date_available = date %in% kd490_GL_dates
  )

#Check data columns
availability_names <- c("l3s_date_available", "chla_GL_date_available", "kd490_GL_date_available")
...
print(dynamic_model_pts %>% summarise(
  ...
  chla = sum(!chla_GL_date_available),
  kd490 = sum(!kd490_GL_date_available))
)
...
dynamic_model_pts <- extract_daily_exact(points_df = dynamic_model_pts, points_sv = dynamic_points_sv, raster_object = chla_GL_r, output_name = "chla_mg_m3")
dynamic_model_pts <- extract_daily_exact(points_df = dynamic_model_pts, points_sv = dynamic_points_sv, raster_object = kd490_GL_r, output_name = "kd490_m_inverse")
...
#Chla missingness
chla_GL_missing_date = sum(!chla_GL_date_available, na.rm = TRUE),
chla_GL_missing_cell = sum(chla_GL_date_available & is.na(chla_mg_m3), na.rm = TRUE),

#Kd490 missingness
kd490_GL_missing_date = sum(!kd490_GL_date_available, na.rm = TRUE),

kd490_GL_missing_cell = sum(kd490_GL_date_available & is.na(kd490_m_inverse), na.rm = TRUE), .groups = "drop")
