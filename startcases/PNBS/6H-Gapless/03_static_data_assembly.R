...
#=================================
crw_dir <- paste0("C:/github/Whale-SDM/", "startcases/PNBS/6H-Gapless/CRW")
...
if (!file.exists(pts_file)) {stop("Combined observed + CRW file not found:\n",pts_file)}
...
#gives model its own and directory
static_dir <- file.path(out_dir, "Static-predictors")
...
#Crop to  extent covering all observed and CRW points in the reduced PNBS space
#Reduced Central California model domain
model_xlim <- c(-123.25, -121.50)
model_ylim <- c(36.25, 37.60)
